...
- Hid non-content parts of the page in print view
- Clear the NavigationDataStore on build id changes
- Consume applications default as class not object
- Shorten preloadData cache lifetime
- Add unsupportedReason to INavigationResult
- Add inplace navigation event
- rename
- Upgrade Typescript to 2.7.2
- Upgrade Typescript to 2.7.2
- Removed unnecessary suite nav configuration reads in base application.
- Upgrade to fabric 6
- Change "loadTheme" import to @ms/uifabric-styling-bundle
...
- reload theme during an SPA transition, even when not using new boot sequence
- Stops loading theme from cache for SP Home.
- api extractor config changes
- graduate NavBelowHeader flight
- Persist app div during app to app transition
- Log correlationId
- add correlationId
- Fixing an issue where using older version of OUFR breaks themes on the page
